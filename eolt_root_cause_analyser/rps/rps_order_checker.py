...
class RpsOrder(FailureModeChecker):
    def _rps_order_checker(self, rps_data: np.ndarray, step_chosen, eol_test_id, test_type):
        ...
        step_details = fetch_step_timings(eol_test_id, test_type)
        step_index = select_step(step_details, rps_data[:, 0], step_chosen)
        ...
        cosP = rps_data[lower_bound:upper_bound, 2]  # cosP from column 2
        sinN = rps_data[lower_bound:upper_bound, 3]  # sinN from column 3
    ...
    def analyse(self):
        ...
        return self._rps_order_checker(rps_data, failurecode_step, self.eol_test_id, self.test_type)
