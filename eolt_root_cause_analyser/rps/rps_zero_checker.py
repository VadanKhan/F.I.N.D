...
class RpsZero(FailureModeChecker):
    def _rps_zero_checker(self, rps_data: np.ndarray):
        """Checks for zero signals in the given RPS data.

        This function takes a NumPy array containing RPS data as input and returns a list of strings indicating the
            status of each sensor. If the RMS value of a sensor is less than or equal to 0.01, its status is "Zero
            Signal", otherwise it is 0.

        Args:
            rps_data (np.ndarray): A NumPy array containing RPS data.

        Returns:
            tuple: A tuple containing two elements: a list of strings indicating the status of each sensor, and a list
            of RMS values for each sensor.
        """
    ...
    def analyse(self):
        ...
        return self._rps_zero_checker(rps_data)
